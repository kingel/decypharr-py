...
export class DownloadPage extends LitElement {
  ...
  _saveOptions() {
    ...
    localStorage.setItem(
      "downloadAction",
      root.getElementById("downloadAction").value,
    );
    localStorage.setItem(
      "downloadUncached",
      root.getElementById("downloadUncached").checked.toString(),
    );
    ...
    localStorage.setItem(
      "downloadFolder",
      root.getElementById("downloadFolder").value,
    );
  }
  ...
  _handleDrop(e) {
    ...
    if (!files.length) {
      window.decypharrUtils?.createToast(
        "Please drop .torrent files only",
        "warning",
      );
      return;
    }
  ...
  async _submit(e) {
    ...
    if (total === 0) {
      window.decypharrUtils?.createToast(
        "Please provide at least one torrent",
        "warning",
      );
      return;
    }
    if (total > 100) {
      window.decypharrUtils?.createToast(
        "Please submit up to 100 torrents at a time",
        "warning",
      );
      return;
    }
    ...
    formData.append(
      "downloadUncached",
      root.getElementById("downloadUncached").checked,
    );
    formData.append(
      "rmTrackerUrls",
      root.getElementById("rmTrackerUrls").checked,
    );
    ...
    this._loading = true;
    try {
      const resp = await fetcher("/api/add", {
        method: "POST",
        body: formData,
        headers: {},
      });
      ...
      if (data.errors && data.errors.length > 0) {
        if (data.results.length > 0) {
          window.decypharrUtils?.createToast(
            `Added ${data.results.length} torrents with ${data.errors.length} errors`,
            "warning",
          );
        ...
        window.decypharrUtils?.createToast(
          `Successfully added ${data.results.length} torrent${data.results.length > 1 ? "s" : ""}!`,
        );
        this._clearForm();
      }
    } catch (err) {
      window.decypharrUtils?.createToast(
        `Error adding downloads: ${err.message}`,
        "error",
      );
    } finally {
      this._loading = false;
          ...
          <form @submit=${this._submit} @change=${this._saveOptions}>
            <div class="grid grid-2">
              ...
              >
                ...
                ${this._fileLabel
                  ? html`<div class="hint">${escapeHtml(this._fileLabel)}</div>`
                  : null}
              </div>
            </div>
