...
class TorrentDashboard {
  ...
  updatePagination() {
    ...
    const endIndex = Math.min(startIndex + this.state.itemsPerPage - 1, this.state.filteredTorrents.length)
    ...
    this.refs.paginationControls.appendChild(
      createButton('«', this.state.currentPage - 1, this.state.currentPage === 1)
    )

    for (let i = 1; i <= totalPages; i++) {
      if (i === 1 || i === totalPages || Math.abs(i - this.state.currentPage) <= 1) {
        this.refs.paginationControls.appendChild(
    ...
    this.refs.paginationControls.appendChild(
      createButton('»', this.state.currentPage + 1, this.state.currentPage === totalPages)
    )
  }
